...
aws s3 cp $PRIMER-$LINKS.fasta s3://$BUCKET/CruxV2/$RUNID/$PRIMER/ecopcr/$LINKS.fasta --no-progress --endpoint-url https://js2.jetstream-cloud.org:8001/
...
for ((nt=0; nt<$NTOTAL; nt++)); do
    nt=$(printf '%03d' $nt)
    ben add -s $BENSERVER -c "docker run --rm -t -v ~/crux/crux/app/blast:/mnt -v ~/crux/crux/vars:/vars -v /tmp:/tmp -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e AWS_DEFAULT_REGION=$AWS_DEFAULT_REGION --name $PRIMER-blast-$LINKS-$nt-$RUNID crux /mnt/blast.sh -c /vars/crux_vars.sh -j $PRIMER-blast-$LINKS-$nt-$RUNID -i $RUNID -p $PRIMER -n $nt -e $LINKS.fasta" $PRIMER-blast-$LINKS-$nt-$RUNID -o /etc/ben/output

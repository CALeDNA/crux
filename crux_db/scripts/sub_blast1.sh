...
while getopts "n:q:o:k:l:d:t:v:i:c:a:j:w:p:q:b:z:y:" opt; do
    case $opt in
        n) NAME="$OPTARG"
        ;;
        q) QU="$OPTARG"
...
${BLASTn_CMD} -query ${QU} -out ${ODIR}/${NAME}_BLAST/${TYPE}_BLAST_out/raw/${FILE}_blast1_out.txt -db ${BLAST_DB} -evalue ${EVAL1:=$BLAST1_eVALUE} -outfmt "6 saccver staxid sseq" -num_threads ${THREAD1:=$BLAST1_NUM_THREADS} -perc_identity ${ID1:=$BLAST1_PERC_IDENTITY} -qcov_hsp_perc ${COV1:=$BLAST1_HSP_PERC} -num_alignments ${RETURN:=$BLAST1_NUM_ALIGNMENTS} -gapopen ${GO:=$GAP_OPEN} -gapextend ${GE:=$GAP_EXTEND}
...
if [ -e "${ODIR}/${NAME}_BLAST/${TYPE}_BLAST_out/fasta/${FILE}_blast1_out.fasta.tmp" ];
then

...
aws s3 cp s3://$BUCKET/crux/$RUNID/fa-taxid/$primer.tax.tsv . --endpoint-url https://js2.jetstream-cloud.org:8001/
# aws s3 cp s3://$BUCKET/tronko/$RUNID/$FOLDER/$FASTA . --endpoint-url https://js2.jetstream-cloud.org:8001/
...
# aws s3 cp $TAXA s3://$BUCKET/tronko/$RUNID/$FOLDER/$TAXA --endpoint-url https://js2.jetstream-cloud.org:8001/

...
def benlist():
  result = subprocess.run(["/etc/ben/ben", "-s", "/tmp/ben-ubuntu", "list"], capture_output=True)
...
def bennodes():
  result = subprocess.run(["/etc/ben/ben", "-s", "/tmp/ben-ubuntu", "nodes"], capture_output=True)
  result = result.stdout.decode().strip().splitlines()
  for i in result:
    i = i.split()
    if len(i) == 4 and i[0] != "#":
